...
class ClipConstants {
  ...
  // 数据库
  static const String databaseName = 'clipflow.db';
  ...
  static const List<String> supportedTextFormats = [
    'txt', 'md', 'json', 'xml', 'html', 'css', 'js', 'dart'
  ];
  
  // 正则表达式
  static const String hexColorPattern = r'^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$';
  static const String rgbColorPattern = r'^rgb\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*\)$';
  static const String hslColorPattern = r'^hsl\(\s*(\d{1,3})\s*,\s*(\d{1,3})%\s*,\s*(\d{1,3})%\s*\)$';
