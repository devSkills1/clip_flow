...
- **双模式界面**：`DynamicHomePage` 可在传统列表与 App Switcher 模式间切换，后者提供模糊背景、横向卡片、键鼠导航与自动隐藏协同。
...
2. **历史记录与模式切换**：`DynamicHomePage` 支持传统列表与 App Switcher 两种布局；前者展示时间线、缩略图、上下文菜单，后者适合全屏快速切换。
